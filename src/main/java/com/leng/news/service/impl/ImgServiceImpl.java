...
public class ImgServiceImpl implements IImgService {
    ...
    public Map<String, Object> layeditImg(MultipartFile file) {
        ...
        if (file != null) {
            ...
            try {
                ...
                // 图片的路径+文件名称
                String fileName = "/upload/img/newsImg/" + substring;
                // 图片的在服务器上面的物理路径
    ...
    public Map<String, Object> newsImg(MultipartFile file) {
        ...
        if (file != null) {
            ...
            try {
                String substring = file.getOriginalFilename();
                // 图片的路径+文件名称
                String fileName = "/upload/img/cover/" + substring;
                // 图片的在服务器上面的物理路径
                File destFile = new File(imgService, fileName);
                // 生成upload目录
                File parentFile = destFile.getParentFile();
    ...
    // 用户头像 图片上传
    public Map<String, Object> userImg(MultipartFile file) {
        Map<String, Object> map = new HashMap<String, Object>();
        Map<String, Object> res = new HashMap<String, Object>();
        if (file != null) {
            String imgService = "D:\\DevTools\\Software\\phpstudy_pro\\Extensions\\Nginx1.16.1\\html";
            try {
                String substring = file.getOriginalFilename();
                // 图片的路径+文件名称
                String fileName = "/upload/img/userImg/" + substring;
                // 图片的在服务器上面的物理路径
                File destFile = new File(imgService, fileName);
                // 生成upload目录
                File parentFile = destFile.getParentFile();
